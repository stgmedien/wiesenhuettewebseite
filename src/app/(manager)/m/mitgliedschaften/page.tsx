  ...
  return (
    <div className="p-8">
      <header className="mb-8">
        ...
        <h1 className="font-heading text-3xl text-[var(--color-wh-forest)]">Mitgliedschaften</h1>
        <p className="text-sm text-[var(--color-wh-charcoal)]/70 mt-1">
          {pending.length} offene Verifizierung{pending.length === 1 ? "" : "en"}.
        </p>
      </header>

      <section className="space-y-3 mb-12">
        <h2 className="font-heading text-xl text-[var(--color-wh-forest)] mb-3">
          Zur Prüfung
        </h2>
        {pending.length === 0 ? (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-[var(--color-wh-charcoal)]/80">{c.email}</p>
                  {c.phone && (
                    <p className="text-sm text-[var(--color-wh-charcoal)]/80">📞 {c.phone}</p>
                  )}
                  {c.memberId && (
                    <p className="text-sm font-mono mt-1 text-[var(--color-wh-charcoal)]">
                      Beantragte Mitgliedsnr.: {c.memberId}
                    </p>
                  )}
                  <p className="text-xs text-[var(--color-wh-charcoal)]/60 mt-2">
                    Beantragt am {formatDateLong(c.createdAt)}
                  </p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3 border-t border-[var(--color-wh-stone)]/40">
                <form action={approveMembership} className="flex gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 rounded-lg border border-[var(--color-wh-stone)] px-3 py-2 text-sm"
                  />
                  <button
                    type="submit"
      ...
      <section className="space-y-3">
        ...
        {recentDecisions.length === 0 ? (
          <div className="text-sm text-[var(--color-wh-charcoal)]/60">Noch keine Bestätigungen.</div>
        ) : (
          <ul className="space-y-2">
            {recentDecisions.map((c) => (
              <li
                key={c.id}
                className="flex items-center justify-between rounded-xl bg-white border border-[var(--color-wh-stone)]/40 px-4 py-3 text-sm"
              >
                <span>
                  <strong>
                    {c.firstName} {c.lastName}
                  ...
                  · {c.email}
                  {c.memberId && (
                    <span className="font-mono text-[var(--color-wh-charcoal)]/70 ml-2">
                      #{c.memberId}
                    </span>
                  )}
                </span>
                <span className="text-xs text-[var(--color-wh-charcoal)]/60">
