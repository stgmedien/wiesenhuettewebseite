...
export const AUTOMATIC_MAIL_TEMPLATES: AutomaticMailTemplate[] = [
  ...
  },
  {
    ...
    label: "Restzahlung bestätigt + Anreise-Infos",
    category: "Zahlungen",
    trigger: "Sofort bei erfolgreicher Restzahlung — automatischer Off-Session-Einzug (T-14) oder Altsystem-Stripe-Link. Enthält Adresse, Hausordnung-Erinnerung und Ankunftszeit-Hinweis (ersetzt die frühere separate T-7-Anreise-Info-Mail).",
    audience: "Gast",
  ...
  },
  {
    ...
    label: "Letzte Erinnerung vor Anreise + Kurkarten",
    category: "Buchung — Gast",
    trigger: "T-3 (3 Tage vor Anreise) — Bitte, Toni die Ankunftszeit mitzuteilen. Zugleich die planmäßige Zustellung von Kurkarten + Feuerwehr-Meldeliste (falls bis dahin hochgeladen). Bei sehr spätem Upload (nach T-3) verschickt kurkarten-upload/route.ts dieselbe Mail sofort als Fallback.",
    audience: "Gast",
    file: "src/app/api/cron/daily-mail-jobs/route.ts, src/app/api/m/kurkarten-upload/route.ts",
  },
  {
    key: "deposit-hold",
  ...
  },
  {
    ...
    category: "Buchung — Hüttenservice/intern",
    trigger: "T-7 — nur noch intern an Toni, keine parallele Gast-Mail mehr (die steckt jetzt in restzahlung-confirmed bei T-14). Ohne Kurkarten-Anhang — die bekommt Toni bei T-3, siehe huettenwart_arrival_reminder.",
    audience: "Toni Klauke + Brandenburg-Team",
    file: "src/app/api/cron/daily-mail-jobs/route.ts",
  },
  {
    key: "huettenwart_arrival_reminder",
    label: "Letzte Erinnerung vor Anreise + Kurkarten — Hüttenservice",
    category: "Buchung — Hüttenservice/intern",
    trigger: "T-3, gleicher Zeitpunkt wie die Erinnerung an den Gast. Zugleich die planmäßige Zustellung von Kurkarten + Feuerwehr-Meldeliste (falls bis dahin hochgeladen). Bei sehr spätem Upload (nach T-3) verschickt kurkarten-upload/route.ts dieselbe Mail sofort als Fallback.",
    audience: "Toni Klauke + Brandenburg-Team",
    file: "src/app/api/cron/daily-mail-jobs/route.ts, src/app/api/m/kurkarten-upload/route.ts",
  },
  {
